...
#define checkResult(succ) if (!(succ)){retVal = false; break;}
		...
		void ObjModel::ConstructPerVertexFaceList(Basic::List<int> & faceCountAtVert, Basic::List<int> & vertFaceList)
		{
			...
			{
				for (int j = 0; j<4; j++)
					if (Faces[i].VertexIds[j] != -1)
						faceCountAtVert[Faces[i].VertexIds[j]]++;
			}
			int scan = 0;
			...
			for (int i = 0; i<Faces.Count(); i++)
			{
				for (int j = 0; j<4; j++)
					if (Faces[i].VertexIds[j] != -1)
						vertFaceList[faceCountAtVert[Faces[i].VertexIds[j]]++] = i;
			}
		}
